...
function buildHeroBlock(main) {
  ...
  // Don't hijack a picture that already belongs to an authored block. At this
  // point blocks aren't decorated yet (no .block class), but the authored block
  // wrapper is the class-bearing div directly under a section's content div, so
  // any class-bearing ancestor div signals the picture is inside a block, not
  // default content. Only auto-build a hero from default-content pictures.
  if (picture && picture.closest('div[class]')) return;
